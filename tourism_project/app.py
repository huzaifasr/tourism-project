...
MODEL_CANDIDATES = [
    'tourism_project/model_building/artifacts/xgb_best_model.joblib',
    'tourism_project/tourism_model_v1.joblib',
    'tourism_project/deployment/tourism_model_v1.joblib',
    'model.joblib',
]
...
try:
    model_path = hf_hub_download(repo_id="huzaifa-sr/tourism_model", filename="tourism_model_v1.joblib", use_auth_token=hf_token)
...
# Prepare categorical dummies to match training preprocessing
cat_cols = ['TypeofContact', 'Occupation', 'ProductPitched', 'MaritalStatus', 'Designation', 'Gender']
try:
    input_dummies = pd.get_dummies(input_df, columns=[c for c in cat_cols if c in input_df.columns], prefix_sep='__', drop_first=False)
except Exception:
    input_dummies = input_df.copy()

# If prepared Xtrain exists, align columns to it (reindex and fill missing with 0)
input_prepared = None
if os.path.exists(PREPARED_X_PATH):
    try:
        ...
        st.warning(f'Failed to read prepared columns from {PREPARED_X_PATH}: {e}')
...
if st.button('Predict'):
    if model is None:
        ...
        try:
            ...
            X = input_prepared if input_prepared is not None else input_df
...
# No automatic local data path shown — uploads are optional and handled in-app
